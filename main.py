...
from TitleWidget import TitleBar
# 枚举左上右下以及四个定点
...
if __name__ == '__main__':
    ...
    with open('./QSS/TitleQSS.qss', 'r') as f:  # 导入QListWidget的qss样式
        app.setStyleSheet(f.read())
    mainWnd = FramelessWindow()
    mainWnd.setWindowTitle('算法可视化')
    mainWnd.setWindowIcon(QIcon('test.png'))
    mainWnd.resize(QSize(1250, 780))
    mainWnd.setWidget(MainWindow(mainWnd))  # 把自己的窗口添加进来
    mainWnd.show()
    mainWnd.titleBar.buttonHome.clicked.connect(mainWnd._widget.left_tag.goHome)
    sys.exit(app.exec_())